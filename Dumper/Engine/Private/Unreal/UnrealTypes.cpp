...
{
	return MemoryAccessor::Read<int32>(Address + Off::FName::CompIdx);
}
...
uint32 FName::GetNumber() const
{
	...
	if (Settings::Internal::bUseNamePool)
		return MemoryAccessor::Read<uint32>(Address + Off::FName::Number); // The number is uint32 on versions <= UE4.23 

	return static_cast<uint32_t>(MemoryAccessor::Read<int32>(Address + Off::FName::Number));
}
